#!/usr/bin/env python3
"""Driver: run ONLY the GROUP 32 reference-MCP-server tests against the live
omnidev API.

tests.py cannot be imported (module-level code references _args defined only
under __main__) and `python3 tests.py --group 32` aborts in the main body at
GROUP 12 (_check_mm_container expects compose project 'omnideploy', but the
dev stack runs as 'omnidev'). Instead, extract the GROUP 32 block verbatim
from tests.py and exec it with the shared test() harness + BASE. Runs only the
seven 32_* tests — no other group's setup side effects."""
import os
import sys
import time
import json
...
# ── extract GROUP 32 block verbatim from tests.py ─────────────────────
src = open("/opt/workspace/omni-deployer/scripts/tests.py", encoding="utf-8").read()
start = src.index("# ── GROUP 32: External / Agnostic MCP Reference Servers")
end = src.index('print("TEST SUMMARY")')
block = src[start:end]

ns = dict(globals())
exec(compile(block, "tests.py[GROUP32]", "exec"), ns)

print("\n" + "=" * 60)
print(f"GROUP 32 SUMMARY: run={tests_run} pass={tests_pass} fail={tests_fail}")
print("=" * 60)
