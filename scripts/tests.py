...
# ═══════════════════════════════════════════════════════════════════════
#  GROUP ISOLATION HARNESS (F1, 2026-09-13)
#
#  Every executable group block of this file is a registered `_seg_<key>()`
#  function, so that
#    * importing this module has NO side effects (it used to run groups 13-55);
#    * one group can be run alone:  --group 37
#    * a run can resume from a group: --start-group 37
#    * the run emits a machine report: --json-report /tmp/report.json
#    * a group may declare prerequisites: --list / --group N --with-prereqs
#  The bodies were NOT changed: the transform only indents them under the
#  function and declares `global` for the names the block assigns (a
#  module-level block assigns module globals, a function body would assign
#  locals). A segment that only binds names (imports/defs/constants) is
#  `always_run` and executes even for a subset selection.
# ═══════════════════════════════════════════════════════════════════════
...
# deploy.py pipes this script on stdin (__file__ == '<stdin>'), so a
# --json-report FILE written inside the container is not reachable from the
# host. The dev failure-resume loop (F2) therefore also prints the run summary
# as ONE stdout marker line, which deploy.py parses to learn which group failed.
REPORT_STDOUT_PREFIX = "OMNIAGENT_TESTS_REPORT "


# compat: the pre-harness bootstrap parsed the CLI into a module-level
# `_args` namespace, which a couple of group blocks still read
# (`if not _args.group: discard_all_changes()`). Keep that name alive.
_args = None


def _cli_parser():
    _p = _argparse.ArgumentParser(
        prog="omniagent-tests",
        description="omniagent integration tests (group-isolated harness)")
    ...
    _p.add_argument("--list", action="store_true",
                    help="Print the group manifest (id, standalone, requires) and exit.")
    _p.add_argument("--json-report", type=str, default="", dest="json_report",
                    help="Write a machine-readable JSON run report to this path.")
    _p.add_argument("--fail-fast", action="store_true", dest="fail_fast",
                    help="Stop at the first failing group (default: run all, report all).")
    _p.add_argument("--allow-dirty", action="store_true", dest="allow_dirty",
                    help="Do not abort on a dirty git tree (resume runs).")
    _p.add_argument("--verify-only", action="store_true", dest="verify_only",
                    help="Full-suite verification pass (the conditional dev second pass); "
                         "runs every group and asserts nothing is filtered.")
    return _p
...
def _print_manifest(segments):
    print("group-isolated manifest (harness v%d): %d segments, %d executable lines" % (
        _HARNESS_VERSION, len(segments), sum(s.get("lines", 0) for s in segments)))
    print("%-7s %-8s %-11s %-9s %-7s %s" % (
        "GROUP", "KEY", "STANDALONE", "REQUIRES", "ALWAYS", "TITLE"))
    for s in segments:
        print("%-7s %-8s %-11s %-9s %-7s %s" % (
            s["id"], s["key"], "yes" if s.get("standalone") else "no",
            ",".join(s.get("requires") or []) or "-",
            "yes" if s.get("always_run") else "no", (s.get("title") or "")[:58]))
    n_alone = len([s for s in segments if s.get("standalone") and not s.get("always_run")])
    print("standalone groups: %d / %d" % (n_alone, len(segments)))


def _select_groups(segments, args):
    """Ordered segment list to run for this invocation."""
    always = [s for s in segments if s.get("always_run")]
    ...
    if args.group:
        ...
        if want in ids or want in keys:
            ...
            print("[selection] --group %r is not a group id -> legacy TEST_FILTER "
                  "name-substring mode" % want)
            sel = ordered
    elif args.start_group:
        want = args.start_group.strip()
        if want not in ids and want not in keys:
            print("ERROR: unknown --start-group %r" % want)
            print("known group ids: %s" % ",".join(ids))
            sys.exit(2)
        idx = ids.index(want) if want in ids else keys.index(want)
        sel = ordered[idx:]
    ...
    return always + [s for s in sel if not s.get("always_run")]


def _run_segments(sel, args):
    results = []
    for spec in sel:
        gid = spec["id"]
        print("\n" + "=" * 60)
        print("GROUP %s: %s" % (gid, spec.get("title") or spec["key"]))
        print("=" * 60)
        before = (tests_run, tests_pass, tests_fail)
        t0 = time.time()
        status, err = "passed", ""
        try:
            spec["fn"]()
            ...
            if _e.code not in (0, None):
                status, err = "error", "SystemExit(%r)" % (_e.code,)
        except BaseException as _e:  # a group must never kill the whole run
            status, err = "error", "%s: %s" % (type(_e).__name__, _e)
            _traceback.print_exc()
        elapsed = time.time() - t0
        d_run = tests_run - before[0]
        d_pass = tests_pass - before[1]
        d_fail = tests_fail - before[2]
        if status == "passed" and d_fail:
            status = "failed"
        if status != "passed" and not err:
            err = "%d test(s) failed" % d_fail
        results.append({
            "id": gid, "key": spec["key"], "title": spec.get("title") or "",
            "status": status, "tests_run": d_run, "tests_pass": d_pass,
            "tests_fail": d_fail, "elapsed_secs": round(elapsed, 3), "error": err,
            "standalone": bool(spec.get("standalone")),
            "requires": list(spec.get("requires") or []),
        })
        print("[GROUP %s] %s - tests +%d (pass +%d, fail +%d) in %s%s" % (
            gid, status.upper(), d_run, d_pass, d_fail, _fmt_secs(elapsed),
            (" - " + err) if err else ""))
        if status != "passed" and args.fail_fast:
            print("[fail-fast] stopping after GROUP %s" % gid)
            break
    return results


def _write_json_report(path, results, sel):
    first_bad = next((r["key"] for r in results if r["status"] != "passed"), None)
    mode = ("verify_only" if _args.verify_only else
            "group" if _args.group else
            "start_group" if _args.start_group else "full")
    data = {
        "schema": "omniagent-tests-report/v1",
        "harness_version": _HARNESS_VERSION,
        "mode": mode,
        "start_group": _args.start_group or "",
        "group": _args.group or "",
        "generated_at": time.time(),
        "argv": sys.argv[1:],
        "selection": [s["key"] for s in sel],
        "groups": results,
        "totals": {
            "groups": len(results),
            ...
            "tests_run": tests_run, "tests_pass": tests_pass, "tests_fail": tests_fail,
        },
    ...
    try:
        ...
        print("[report] wrote %s (ok=%s, first_failure=%s)" % (
            path, data["ok"], first_bad))
        print("%s%s" % (REPORT_STDOUT_PREFIX, json.dumps({
            k: data[k] for k in ("schema", "mode", "start_group", "group",
                                 "ok", "first_failure", "totals")})))
    except OSError as e:
        print("[report] WARNING: could not write %s: %s" % (path, e))


def _run_main(segments):
    global _args
    args = _cli_parser().parse_args()
    _args = args
    if args.list:
        _print_manifest(segments)
        sys.exit(0)
    # check_git_clean() runs in ALL modes now (it used to be skipped when a
    # --group filter was given): a dirty tree is a real failure source.
    # Resume runs pass --allow-dirty because they intentionally continue a run.
    if not args.allow_dirty:
        check_git_clean()
    import subprocess as _git_sp
    _git_sp.run(["git", "config", "--global", "--add", "safe.directory",
                 "/opt/workspace/omni-stack"], capture_output=True, timeout=10)
    try:
        _r = urllib.request.urlopen(f"{BASE}/health", timeout=5)
        assert _r.status == 200
        print(f"API healthy at {BASE}\n")
    except Exception as _e:
        print(f"API not accessible: {_e}")
        sys.exit(1)
    if args.json_report and os.path.exists(args.json_report):
        try:
            os.remove(args.json_report)
        except OSError:
            pass
    if args.verify_only:
        print("[verify-only] conditional full verification pass: every group runs")
    sel = _select_groups(segments, args)
    t0 = time.time()
    results = _run_segments(sel, args)
    total = time.time() - t0
    print("\n" + "=" * 60)
    print("  GROUP SUMMARY (%d segment(s) run, %s)" % (len(results), _fmt_secs(total)))
    print("=" * 60)
    for _r in results:
        print("  %-7s %-8s tests=%d pass=%d fail=%d %s" % (
            _r["id"], _r["status"], _r["tests_run"], _r["tests_pass"],
            _r["tests_fail"], _fmt_secs(_r["elapsed_secs"])))
    print("  TOTAL tests=%d pass=%d fail=%d" % (tests_run, tests_pass, tests_fail))
    if args.json_report:
        _write_json_report(args.json_report, results, sel)
    bad = [r for r in results if r["status"] != "passed"]
    if bad:
        print("\nFAILED GROUPS: %s" % ", ".join(
            "%s(%s)" % (r["key"], r["status"]) for r in bad))
    sys.exit(0 if not bad else 1)


def _seg_1():
    """GROUP 1: Original Remove API tests (idempotent"""
    global fn
...
def _seg_5():
    """GROUP 5: Dashboard page loading tests"""
    global fn
...
def _seg_11():
    """GROUP 11: Prompt Plugin Tests"""
    global _ex, attempt, body, e, fn, r, resp, t, time, tools, tools_data
...
def _seg_12():
    """GROUP 12: File Upload via Mattermost + test-tool-caller"""
    global MM, admin_data, admin_req, admin_token, ch, channels, cid, mm_channel_id, name, resp, setup_req, setup_resp, t, team_id, team_resp, val
...
def _seg_13B():
    """GROUP 13B: BG task single-execution regression (no re-send"""
    global test_fn_13b_bg_task_single_execution
...
def _seg_15():
    """GROUP 15: Settings API (lower_snake_case"""
    global test_fn_15_settings_env_ref, test_fn_15_settings_hardcoded, test_fn_15_settings_readonly_bootstrap, test_fn_15_settings_secret_ref, test_fn_15_settings_update
...
def _seg_16():
    """GROUP 16: Tool/multi-tool/tool-result message format verification"""
...
def _seg_17():
    """GROUP 17: Parallel wait 3 tools x 50 calls x 30s (concurrent, 30-40s"""
...
def _seg_17B():
    """GROUP 17B: Parallel docker_compose exec 50 x 30s (concurrent, <60s"""
...
def _seg_17C():
    """GROUP 17C: Parallel filesystem MCP calls (read/list/search/info/grep"""
...
def _seg_18():
    ...
    global test_fn_18_platform_multi_source
...
def _seg_23():
    ...
    global _remote_yml_entry, ensure_remote_plugin_from, test_23_1_import_several_from_remote_yml, test_23_2_import_from_remote_test_yml, test_23_3_override_remote_plugin, test_23_4_remove_remote_plugins, test_23_5_ci_foreign_owner_mirror_clone
...
def _seg_24():
    ...
    global _g24_assistant_msg, _g24_mcp_execute, _g24_size, _g24_tool_msg, _g24_wait_for_tool, test_fn_24_compact_keeps_result_excerpt, test_fn_24_read_offset_limit
...
def _seg_25():
    ...
    global _, _g25_toolbox_db, _g25_toolbox_name, i, t, telapsed, test_fn_25_db_vectorizer, test_fn_25_search_wiki, tname, total
...
def _seg_19():
    ...
    global _get_platform_detail, _get_platform_source, _get_platform_status, _platform_subprocess_running, _wait_for_platform_status, _wait_for_platform_subprocess, test_fn_19_ensure_disabled, test_fn_19_platform_enable_non_existent, test_fn_19_platform_lifecycle
    # ═══════════════════════════════════════════════════════════════════════
    #  GROUP 19: Platform Plugin Lifecycle (subprocess start/stop verification)
    # ═══════════════════════════════════════════════════════════════════════
...
def _seg_20():
    ...
    global _action_id, _kanban_task_id, _schedule_id, tasks_yml_remove_keys, test_20_10_actions_crud, test_20_11_platforms, test_20_12_mcp_tools, test_20_13_memory, test_20_14_plugins_ping, test_20_15_threads_list, test_20_16_channel_invalid_id, test_20_1_health, test_20_2_channels, test_20_3_threads, test_20_4_messages, test_20_5_overview, test_20_6_settings, test_20_7_kanban_crud, test_20_7b_kanban_plan_normalized, test_20_8_schedule_crud, test_20_9_secrets_crud
...
def _seg_21():
    ...
    global test_21_1_noop_provider_lifecycle
...
def _seg_22():
    ...
    global WF_SCRIPT_4STEPS, WF_SCRIPT_FAIL_RUNNING, WF_SCRIPT_FAIL_TESTING, WF_SCRIPT_OK, _wf_bootstrap_trunc_channel, _wf_channel_restore, _wf_cleanup, _wf_create_task, _wf_ensure_test_python, _wf_history_retry_fired, _wf_history_rows, _wf_remove_test_python, _wf_settings_get, _wf_step_threads, _wf_task_status, _wf_wait_status, i, test_22_1_unknown_route, test_22_2_nonexistent_schedule, test_22_3_kanban_missing_title, test_22_4_nonexistent_action, test_22_5_nonexistent_secret, test_22_6_kanban_invalid_status, test_22_workflow_10_truncated_review_not_done, test_22_workflow_1_executor_only, test_22_workflow_2_executor_tester, test_22_workflow_3_executor_tester_reviewer, test_22_workflow_4_fail_thread_running_retry_then_blocked, test_22_workflow_5_fail_thread_testing_no_tester_blocked, test_22_workflow_6_interruption_rerun, test_22_workflow_7_clear_executions_on_review, test_22_workflow_8_d9_dependency_gate, test_22_workflow_9_dispatch_channel_busy_gate
...
def _seg_26():
    ...
    global _p_create_plain_task, test_26_plain_kanban_terminal_clean_completion_review, test_26_plain_kanban_terminal_fail_thread_blocked
    #  GROUP 26: Plain kanban task (NO workflow_id) - fail-tool -> blocked; clean completion -> review (R8-N)
    print(f"\n{'=' * 60}")
...
def _seg_27():
    ...
    global _h27_api, _h27_cleanup, _h27_counter, _h27_counter_key, _h27_create_hook, _h27_get_raw, _h27_logs, _h27_new_threads, _h27_nonhook_ground, _h27_obs_ge, _h27_pre_threads, _h27_quiesce, _h27_run_cron, _h27_sql, _h27_wait_http, _h27_wait_http_raw, _h27_wait_until, _h27f_meta, _h27f_new, _h27f_pre, test_27_hooks_counter_trigger_reset, test_27_hooks_error_isolation, test_27_hooks_event_action, test_27_hooks_event_meta, test_27_hooks_infinite_loop_protection, test_27_hooks_scope_channel_profile, test_27_hooks_thread_finished
...
def _seg_28():
    ...
    global test_28_terminal_status_invariant
...
def _seg_29():
    ...
    global _g29_assert_thread_status_marker, _g29_cleanup_threads, _g29_insert_pending_thread, _g29_kanban_thread_status, _g29_make_task, _g29_patch_status, _g29_redispatch, _g29_threads, test_29_redispatch_endpoint, test_29_status_change_dispatch_review, test_29_status_change_dispatch_running, test_29_status_change_dispatch_testing_skips_stale, test_29_status_change_no_workflow_noop
...
def _seg_30():
    ...
    global _g30_channel, _g30_delete_channel, _g30_delete_thread_ids, _g30_handler_running, _g30_insert_thread, _g30_live_find_thread, _g30_make_task, _g30_max_thread_id, _g30_msg_count, _g30_status, _g30_task, test_30_stop_thread_kanban_clears_thread_status, test_30_stop_thread_live_pending_stop_keeps_processing, test_30_stop_thread_pending_never_cancels_handler, test_30_stop_thread_processing_cancels_handler_and_respawns
...
def _seg_32():
    ...
    global _g32_healthy_call, _g32_is_error, _g32_mcp_execute, _g32_mcp_execute_raw, _g32_servers_present, test_32_everything_echo, test_32_failure_is_plugin_error_not_hang, test_32_fetch_raw, test_32_filesystem_hang_timeout_regression, test_32_filesystem_list_allowed, test_32_git_status, test_32_memory_create_entities, test_32_parallel_calls_all_servers, test_32_sequentialthinking, test_32_time_get_current_time
...
def _seg_33():
    ...
    global TG_DIR, _g33_call, _g33_free_port, _g33_mock_get, _g33_mock_post, _g33_notification, _g33_platform_proc, _g33_socket, _g33_start_mock, _g33_stop_proc, _g33_subprocess, test_33_telegram_errors_mock, test_33_telegram_inbound_mock, test_33_telegram_outbound_mock, test_33_telegram_poll_survives_api_failures, test_33_telegram_shutdown_cleanup, test_33_telegram_slow_api_bounded, test_33_telegram_stdout_json_lines
...
def _seg_34():
    """GROUP 34: """
    global _g34_BASE, _g34_NEXT_ID, _g34_REAL, _g34_SHIM_DIR, _g34_SHIM_SCRIPT, _g34_SSHD, _g34_SSH_BIN, _g34_STDERR, _g34_authorize, _g34_bin, _g34_call, _g34_configure, _g34_devnull_usable, _g34_ensure_sshd, _g34_free_port, _g34_init, _g34_keygen, _g34_prepare, _g34_select, _g34_setup_ssh_dir, _g34_sftp_server, _g34_shutil, _g34_socket, _g34_spawn, _g34_spawn_plugin, _g34_start_sshd, _g34_stop_proc, _g34_subprocess, _g34_test_dir, _g34_tool, _g34_write, test_34_ssh_copy, test_34_ssh_errors, test_34_ssh_run
...
def _seg_35():
    ...
    global WF_SCRIPT_SUBTASKS, test_35_subtasks_plan_mode_lifecycle
...
def _seg_36():
    ...
    global _g36_ensure_paperclip_installed, _g36_installed_dir, _g36_paperclip_dir, test_36_compose_service, test_36_config_wiring, test_36_deploy_seed, test_36_dev_overlay, test_36_live_health, test_36_mcp_stdio_tools, test_36_plugin_files
...
def _seg_37():
    ...
    global test_37_config_wiring, test_37_live_actions, test_37_live_plugin_status, test_37_mcp_stdio_tools, test_37_plugin_files
...
def _seg_38():
    ...
    global _g38_create_demo_skill, _g38_find_binary, _g38_init, _g38_spawn, _g38_tool, test_38_prompt_renders_skills_block, test_38_skills_create_list_view
...
def _seg_39():
    ...
    global test_39_live_plugins, test_39_omniagent_api_generic_tool, test_39_plugins_yml_consolidated, test_39_schedule_delete, test_39_search_tools_listed
...
def _seg_40():
    ...
    global test_40_action_executor_fail_blocked, test_40_action_executor_success, test_40_action_reviewer_fail_blocked, test_40_action_tester_fail_review, test_40_auto_approve_done_direct, test_40_auto_approve_forces_review_on_fail_false, test_40_review_on_fail_goes_review
...
def _seg_41():
    ...
    global WF_SCRIPT_FAIL_BLOCKED, WF_SCRIPT_FAIL_F0, WF_SCRIPT_FAIL_REASON, _wf41_roles_exec_action_tester_agent, _wf41_wait_retry, test_41_auto_approve_forces_review_on_fail_false, test_41_blocked_restriction, test_41_executor_f0_rerun_vs_review, test_41_explicit_running_honored_both_flags, test_41_fail_reason_propagates_to_rerun_cause, test_41_retry_limit_flag_true_review, test_41_tester_f0_rerun_vs_review
...
def _seg_42():
    ...
    global _g42_spawn, test_42_custom_omni_dir_config, test_42_source_audit, test_42_unset_omni_dir_errors
...
def _seg_43():
    ...
    global OMNIAGENT_SRC, _g43_read, _g43_settings_map, test_43_appendable_pending_sql, test_43_db_schema, test_43_settings_api, test_43_settings_regroup, test_43_source_audit
    ...
    def test_43_source_audit():
        ...
        _gsec = settings.split('"general",', 1)
        assert len(_gsec) == 2, "general section list missing from settings.rs"
        _gblock = _gsec[1].split("]),", 1)[0]
        for _k in ("delete_after_days_soft", "delete_after_days_hard"):
            assert _k in _gblock, f"{_k} must be categorized under general"
    ...
    def test_43_settings_regroup():
        ...
        for _ret in ("delete_after_days_soft", "delete_after_days_hard"):
...
def _seg_44():
    ...
    global test_44_fetch_method_gating, test_44_plugin_endpoint_via_builtin_tool, test_44_tool_caller_omniagent_api
...
def _seg_helpers():
    """GROUP helpers: shared load-path helpers (group 45/53"""
    global _cleanup_seeded, _live_tool_names, _makedirs_tracked, _mcp_execute_tool, _mcp_text, _pick_live_tool, _seed_wiki_page
...
def _seg_45():
    ...
    global test_45_guidance, test_45_skill_file, test_45_wiki_live_smoke
    # ==============================================================
    #  GROUP 45: Wiki data source skill (task_18cd39ea0c185171) - skill file,
...
def _seg_46():
    ...
    global G46_MODELS_YML, _g46_data, _g46_post_status, _g46_providers_from_plugins, _g46_read_models, _g46_write_models, backup_models_yml, restore_models_yml, test_46_absent_file, test_46_models_crud, test_46_pluginless_provider, test_46_refresh_upsert
...
def _seg_48():
    ...
    global OMNIAGENT_BIN, _g48_acquire_lock, test_48_advisory_lock_refuses_second, test_48_cli_help, test_48_cli_unknown, test_48_cli_version
...
def _seg_49():
    ...
    global DASHBOARD_REPO, _g49_read, test_49_backend_serves_search_database, test_49_db_tables_search_database, test_49_explorer_git_box_bottom, test_49_hooks_modal, test_49_kanban_board_custom_selects, test_49_plugin_remove_non_builtin, test_49_red_cancel_opaque_modal, test_49_templates_all_profiles_sorted, test_49_workflow_options
...
def _seg_50():
    """GROUP 50: """
    global DEPLOYER_REPO, PUSH_TAG, _make_fixture_workspace, tempfile, test_50_dashboard_connected_version_renders, test_50_dashboard_health_fallback, test_50_push_tag_matching, test_50_push_tag_mismatch_aborts, test_50_push_tag_verify_before_any_git_op, test_50_real_repos_versions_agree
...
def _seg_51():
    ...
    global _g51_get_setting, _g51_mm_channel, _g51_post_and_collect, _g51_put_setting, test_51_redaction_tool
    # ── GROUP 51: Secret redaction via omni-plugins redaction tool ────────────
...
def _seg_52():
    ...
    global test_52_env_leak_marker
...
def _seg_53():
    ...
    global G53_TOOLS, _g53_chars, _g53_present, _g53_ref, test_53_docs_and_baseline_recorded, test_53_failure_bounded_not_hang, test_53_form_auth_flow_token_baseline, test_53_navigate_snapshot, test_53_registration
...
def _seg_54():
    ...
    global _g54, _g_ext_script, test_54_recipe_static, test_54_secrets_redaction, test_54_session_negative_control, test_54_storage_state_roundtrip
    # ---- GROUP 54: web session/auth recipe (external plan X5, D-2) ----
...
def _seg_55():
    ...
    global _g55, test_55_a_prereqs_and_sms_deferral, test_55_b_himalaya_timeout_hang_failure_parallel_cleanup, test_55_c_totp_vector_failure_parallel_cleanup, test_55_d_playwright_hang_bounded_and_parallel, test_55_e_tool_timeout_bounded_and_cleanup, test_55_f_no_leak_and_stack_healthy
...
_GROUP_SEGMENTS = [
    {"id": '1', "key": '1', "title": 'Original Remove API tests (idempotent', "standalone": True, "requires": [], "always_run": False, "line": 5313, "lines": 12, "fn": _seg_1},
    {"id": '2', "key": '2', "title": 'Source-aware Remove API tests', "standalone": True, "requires": [], "always_run": False, "line": 5327, "lines": 5, "fn": _seg_2},
    {"id": '3', "key": '3', "title": 'File upload tests', "standalone": True, "requires": [], "always_run": False, "line": 5334, "lines": 5, "fn": _seg_3},
    {"id": '4', "key": '4', "title": 'Source-required validation tests', "standalone": True, "requires": [], "always_run": False, "line": 5341, "lines": 5, "fn": _seg_4},
    {"id": '5', "key": '5', "title": 'Dashboard page loading tests', "standalone": True, "requires": [], "always_run": False, "line": 5348, "lines": 5, "fn": _seg_5},
    {"id": '6', "key": '6', "title": 'Comprehensive Plugin Action Tests', "standalone": True, "requires": [], "always_run": False, "line": 5355, "lines": 33, "fn": _seg_6},
    {"id": '7', "key": '7', "title": 'Memory Edit/Upload Tests', "standalone": True, "requires": [], "always_run": False, "line": 5391, "lines": 15, "fn": _seg_7},
    ...
    {"id": '14', "key": '14', "title": 'Cancel Task via test-tool-caller', "standalone": True, "requires": [], "always_run": False, "line": 5795, "lines": 4, "fn": _seg_14},
    {"id": '15', "key": '15', "title": 'Settings API (lower_snake_case', "standalone": True, "requires": [], "always_run": False, "line": 5801, "lines": 181, "fn": _seg_15},
    {"id": '16', "key": '16', "title": 'Tool/multi-tool/tool-result message format verification', "standalone": True, "requires": [], "always_run": False, "line": 6006, "lines": 4, "fn": _seg_16},
    {"id": '17', "key": '17', "title": 'Parallel wait 3 tools x 50 calls x 30s (concurrent, 30-40s', "standalone": True, "requires": [], "always_run": False, "line": 6012, "lines": 4, "fn": _seg_17},
    {"id": '17B', "key": '17B', "title": 'Parallel docker_compose exec 50 x 30s (concurrent, <60s', "standalone": True, "requires": [], "always_run": False, "line": 6018, "lines": 4, "fn": _seg_17B},
    {"id": '17C', "key": '17C', "title": 'Parallel filesystem MCP calls (read/list/search/info/grep', "standalone": True, "requires": [], "always_run": False, "line": 6024, "lines": 5, "fn": _seg_17C},
    {"id": '18', "key": '18', "title": 'Multi-source Platform Plugin Tests (Python, JS, Rust', "standalone": True, "requires": [], "always_run": False, "line": 6032, "lines": 183, "fn": _seg_18},
    {"id": '23', "key": '23', "title": 'Remote plugin import (remote.yml / remote.test.yml', "standalone": True, "requires": [], "always_run": False, "line": 6243, "lines": 194, "fn": _seg_23},
    {"id": '24', "key": '24', "title": 'regression tests for (1) prompt compaction keeping a content', "standalone": False, "requires": ['11'], "always_run": False, "line": 6463, "lines": 125, "fn": _seg_24},
    {"id": '25', "key": '25', "title": 'DB vectorizer + search_wiki', "standalone": False, "requires": ['24'], "always_run": False, "line": 6605, "lines": 166, "fn": _seg_25},
    {"id": '19', "key": '19', "title": 'Platform Plugin Lifecycle (subprocess start/stop verification', "standalone": False, "requires": ['11'], "always_run": False, "line": 6792, "lines": 167, "fn": _seg_19},
    {"id": '20', "key": '20', "title": 'API CRUD Integration Tests', "standalone": True, "requires": [], "always_run": False, "line": 7002, "lines": 346, "fn": _seg_20},
    {"id": '21', "key": '21', "title": 'Noop Provider & Executor Verification', "standalone": True, "requires": [], "always_run": False, "line": 7413, "lines": 36, "fn": _seg_21},
    {"id": '22', "key": '22', "title": 'Edge Cases & Error Handling', "standalone": False, "requires": ['11'], "always_run": False, "line": 7456, "lines": 613, "fn": _seg_22},
    {"id": '26', "key": '26', "title": 'Plain kanban task (NO workflow_id) - fail-tool -> blocked; clean completion -> review (R8-N', "standalone": False, "requires": ['11', '22'], "always_run": False, "line": 8140, "lines": 84, "fn": _seg_26},
    {"id": '27', "key": '27', "title": 'Event-driven Hooks system (omniagent 9797aa6', "standalone": False, "requires": ['11', '20'], "always_run": False, "line": 8232, "lines": 574, "fn": _seg_27},
    {"id": '28', "key": '28', "title": 'Terminal status invariant (task_18cb83096b238872', "standalone": False, "requires": ['27'], "always_run": False, "line": 8868, "lines": 68, "fn": _seg_28},
    {"id": '29', "key": '29', "title": 'Kanban status-change dispatch + /redispatch (task_18cbc3b0765efa85', "standalone": False, "requires": ['22', '27'], "always_run": False, "line": 8945, "lines": 238, "fn": _seg_29},
    {"id": '30', "key": '30', "title": 'Surgical stop-thread (task_18cbcd7a8c4a6f5e', "standalone": False, "requires": ['11', '13', '22', '27'], "always_run": False, "line": 9217, "lines": 325, "fn": _seg_30},
    {"id": '31', "key": '31', "title": 'Kanban Boards (config/boards.yml) - task_18cc48e8eace4df3 ──────', "standalone": False, "requires": ['27'], "always_run": False, "line": 9587, "lines": 237, "fn": _seg_31},
    {"id": '32', "key": '32', "title": 'External / Agnostic MCP Reference Servers - task_18cc528e459bcad0 ──', "standalone": False, "requires": ['11'], "always_run": False, "line": 9849, "lines": 266, "fn": _seg_32},
    {"id": '33', "key": '33', "title": 'Python Telegram Platform Plugin (mock Bot API) - task_18cc528e459bcad0 ──', "standalone": False, "requires": ['11'], "always_run": False, "line": 10152, "lines": 405, "fn": _seg_33},
    {"id": '34', "key": '34', "title": '', "standalone": False, "requires": ['11'], "always_run": False, "line": 10610, "lines": 494, "fn": _seg_34},
    {"id": '35', "key": '35', "title": 'Subtasks lifecycle - plan-mode workflow drives', "standalone": False, "requires": ['11', '22', '27'], "always_run": False, "line": 11196, "lines": 70, "fn": _seg_35},
    {"id": '36', "key": '36', "title": 'Paperclip service + official MCP server integration -', "standalone": False, "requires": ['34'], "always_run": False, "line": 11275, "lines": 234, "fn": _seg_36},
    {"id": '37', "key": '37', "title": 'Remote python actions plugin (omni-plugins tools/actions) -', "standalone": False, "requires": ['11', '34'], "always_run": False, "line": 11536, "lines": 211, "fn": _seg_37},
    {"id": '38', "key": '38', "title": 'Skills plugin lifecycle - task_18cc76881db8a89a', "standalone": False, "requires": ['34'], "always_run": False, "line": 11761, "lines": 150, "fn": _seg_38},
    {"id": '39', "key": '39', "title": 'Plugin consolidation - task_18cc76a266a194f9', "standalone": False, "requires": ['20', '23', '24'], "always_run": False, "line": 11932, "lines": 162, "fn": _seg_39},
    {"id": '40', "key": '40', "title": 'Workflow role mode (agent/action) + auto_approve +', "standalone": False, "requires": ['11', '12', '22'], "always_run": False, "line": 12113, "lines": 216, "fn": _seg_40},
    {"id": '41', "key": '41', "title": 'Fail-thread routing (review_on_fail) + double-normalization fix', "standalone": False, "requires": ['11', '22'], "always_run": False, "line": 12347, "lines": 236, "fn": _seg_41},
    {"id": '42', "key": '42', "title": 'Plugins omni_dir config field (task_18cd0c7a02d3884f) -', "standalone": False, "requires": ['34', '38'], "always_run": False, "line": 12606, "lines": 151, "fn": _seg_42},
    {"id": '43', "key": '43', "title": 'Sub-prompts - pending user prompts appended to running thread', "standalone": True, "requires": [], "always_run": False, "line": 12773, "lines": 249, "fn": _seg_43},
    {"id": '44', "key": '44', "title": 'builtin omniagent-api via test-tool-caller + fetch method gating ──', "standalone": False, "requires": ['24'], "always_run": False, "line": 13041, "lines": 132, "fn": _seg_44},
    {"id": 'helpers', "key": 'helpers', "title": 'shared load-path helpers (group 45/53', "standalone": False, "requires": ['24'], "always_run": True, "line": 13181, "lines": 87, "fn": _seg_helpers},
    {"id": '45', "key": '45', "title": 'Wiki data source skill (task_18cd39ea0c185171) - skill file,', "standalone": False, "requires": ['11', '24', 'helpers'], "always_run": False, "line": 13283, "lines": 117, "fn": _seg_45},
    {"id": '46', "key": '46', "title": 'models.yml provider/model overrides (task_18cd408ead8bcbbd', "standalone": True, "requires": [], "always_run": False, "line": 13407, "lines": 242, "fn": _seg_46},
    {"id": '47', "key": '47', "title": 'Resolve fallback fields ONCE at load - kanban task defaults', "standalone": True, "requires": [], "always_run": False, "line": 13665, "lines": 375, "fn": _seg_47},
    {"id": '48', "key": '48', "title": 'Single-instance advisory lock + CLI arg handling', "standalone": True, "requires": [], "always_run": False, "line": 14076, "lines": 75, "fn": _seg_48},
    {"id": '49', "key": '49', "title": 'omni-dashboard UI/UX fixes - 8 items (task_18cd65247cfc4d9e', "standalone": True, "requires": [], "always_run": False, "line": 14161, "lines": 155, "fn": _seg_49},
    {"id": '50', "key": '50', "title": '', "standalone": False, "requires": ['49'], "always_run": False, "line": 14332, "lines": 126, "fn": _seg_50},
    {"id": '51', "key": '51', "title": 'Secret redaction via omni-plugins redaction tool ────────────', "standalone": False, "requires": ['11'], "always_run": False, "line": 14484, "lines": 145, "fn": _seg_51},
    {"id": '52', "key": '52', "title": 'Platform-level env isolation (no ambient env leak to children) ----', "standalone": False, "requires": ['11'], "always_run": False, "line": 14648, "lines": 104, "fn": _seg_52},
    {"id": '53', "key": '53', "title": 'Playwright MCP remote tool plugin (external plan X4) ----', "standalone": False, "requires": ['11', '32', 'helpers'], "always_run": False, "line": 14757, "lines": 181, "fn": _seg_53},
    {"id": '54', "key": '54', "title": 'web session/auth recipe (external plan X5, D-2) ----', "standalone": True, "requires": [], "always_run": False, "line": 14958, "lines": 48, "fn": _seg_54},
    {"id": '55', "key": '55', "title": 'external-tool robustness (external plan X6; code plan 6.2-6.4) ----', "standalone": False, "requires": ['54'], "always_run": False, "line": 15021, "lines": 39, "fn": _seg_55},
]

if __name__ == "__main__":
    _run_main(_GROUP_SEGMENTS)
