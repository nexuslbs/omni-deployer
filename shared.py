...
def seed_remote_plugins():
    ...
    if not isinstance(parsed, dict):
        parsed = {}
    entries = []
    for plugin_type, group in parsed.items():
        if not isinstance(group, dict):
            continue
        for name, meta in group.items():
            if not isinstance(meta, dict):
                continue
            entries.append((name, plugin_type, meta))
    print(f"\n[Seeding remote plugins from {remote_yml}...]")
    ...
    for name, plugin_type, meta in sorted(entries):
        ...
        if "/" in path:
            _pt, path_tail = path.split("/", 1)
        else:
            _pt, path_tail = plugin_type, path
        install_path = f"{plugin_type}/{path_tail}"
        # file:// first when the plugin's repo is the local omni-plugins
        # checkout (fast, offline); mcp-* reference servers live in a separate
        # upstream repo, so they only get their own HTTPS url.
        candidates = [url]
        if url.startswith("file://") or "nexuslbs/omni-plugins" in url:
            if os.path.isdir(CONTAINER_REMOTE):
        ...
        for c in candidates:
            if c not in seen:
                ...
                uniq.append(c)
        ok = False
        for install_url in uniq:
            try:
                payload = json.dumps({"url": install_url, "name": name, "path": install_path})
                oc_write("/tmp/_seed_remote.json", payload)
                r = oc("curl -sf -X POST http://localhost:8080/api/plugins/install-git "
                       "-H 'Content-Type: application/json' -d @/tmp/_seed_remote.json")
                if r.returncode == 0:
                    print(f"  ✓ seeded {plugin_type}/{name} via {install_url}")
                    ok = True
                    break
                low = (r.stdout + r.stderr).lower()
                if "already" in low:
                    print(f"  ✓ {plugin_type}/{name} already registered (skipped)")
                    ok = True
                    break
                print(f"  ~ {plugin_type}/{name} via {install_url}: {low[-80:]}")
            except Exception as e:
                print(f"  ~ {plugin_type}/{name} via {install_url}: {str(e)[:80]}")
        if ok:
            installed += 1
        else:
